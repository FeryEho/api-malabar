...
<?php
	...
	if (isset($_POST['btnAdd'])) {
		...
		if (!empty($NamaBank) && !empty($NoRekening) && empty($error['image'])) {

            $string = '0123456789';
            $file = preg_replace("/\s+/", "_", $_FILES['image']['name']);
            $function = new functions;
            $image = $function->get_random_string($string, 4)."-".date("Y-m-d").".".$extension;
            $upload = move_uploaded_file($_FILES['image']['tmp_name'], 'upload/imagebank/'.$image);        	
            $upload_image = $image;

			// insert new data to menu table
			$sql_query = "INSERT INTO daftarbank (NamaBank, NoRekening, image, NamaPemilikRekening) VALUES (?, ?, ?, ?)";
					
			$stmt = $connect->stmt_init();
			if ($stmt->prepare($sql_query)) {	
				// Bind your variables to replace the ?s
				$stmt->bind_param('ssss', $NamaBank, $NoRekening, $upload_image, $NamaPemilikRekening);
				// Execute query
				$stmt->execute();
				// store result 
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="form-group pmd-textfield">
								...
								<input type="text" name="NamaBank" id="NamaBank" class="form-control" placeholder="" required>
							</div>
						</div>
					</div>
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="form-group pmd-textfield">
								...
								<input type="text" name="NoRekening" id="NoRekening" class="form-control" placeholder="" required>
							</div>
						</div>
					</div>
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="form-group pmd-textfield">
								...
								<input type="text" name="NamaPemilikRekening" id="NamaPemilikRekening" class="form-control" placeholder="">
							</div>
						</div>
					</div>
